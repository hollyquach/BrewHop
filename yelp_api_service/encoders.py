...
def convert_hours(day_hours):
    """
    Given a dictionary of day, start & end from
    Yelp API Business open hours details,
    returns a string with the week day
    and open hours (12 hour format)
    """
    days = [
        "Monday",
        "Tuesday",
    ...
    start = time.strftime(
        "%I:%M %p", time.strptime(day_hours["start"], "%H%M")
    )
    end = time.strftime(
        "%I:%M %p", time.strptime(day_hours["end"], "%H%M")
    )
